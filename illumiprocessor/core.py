...
class CreateDir(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if os.path.exists(d):
            answer = input("[WARNING] Output directory exists, REMOVE [Y/n]? ").upper()
            if answer == "Y":
                shutil.rmtree(d)
            else:
                print("[QUIT]")
...
class SequenceData:
    ...
    def _get_read_data(self):
        all_reads = glob.glob(
            os.path.join(self.input_dir, "*{}*".format(self.start_name))
        )
        ...
        if not self.se and (self.r1 == () or self.r2 == ()):
            raise IOError(
                "There is a problem with the read names for {}. "
...
def trimmomatic_se_runner(work):
    ...
    for read in ["READ1"]:
        input.append(
            os.path.join(
                sample.raw_reads, "{}-{}.fastq.gz".format(sample.end_name, read)
            )
        )
    output = []
    for read in ["READ1"]:
...
def trimmomatic_merger(sample):
    ...
    # remove READ2-single
    with open(new_pth, "ab") as outfile, open(singles[1], "rb") as add_to:
        outfile.write(add_to.read())
    os.remove(singles[1])
    ...
    for sample in reads:
        ...
        for reads in sample.r1:
            new_file = os.path.join(
                sample.raw_reads, "{}-READ1.fastq.gz".format(sample.end_name)
            )
            os.symlink(reads, new_file)
        for reads in sample.r2:
            new_file = os.path.join(
                sample.raw_reads, "{}-READ2.fastq.gz".format(sample.end_name)
            )
            os.symlink(reads, new_file)
